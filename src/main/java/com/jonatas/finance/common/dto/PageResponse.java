...
public record PageResponse<TData>(
    ...
    public record PageMetadata(
                               @Schema(example = "1", description = "pagina atual") Integer page,
                               @Schema(example = "20", description = "tamanho da pagina") Integer size,
                               @Schema(example = "100", description = "total de registros da consulta") Integer totalElements,
                               @Schema(example = "5", description = "pagina atual") Integer totalPages) {
    }

    public static <TData> PageResponse<TData> from(Page<TData> page) {
        return new PageResponse<>(
                LocalDateTime.now(), 200, page.getContent(), new PageMetadata(
