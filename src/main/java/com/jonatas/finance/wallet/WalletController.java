...
public class WalletController {
  ...
  public ResponseEntity<?> create(
    ...
    if (result instanceof CreateWalletResult.AlreadyExistsWalletWithThisName) {
      var error =
          new Error<>("wallet_already_exists", "Already exists an wallet register with same name");
    ...
    var location =
        UriComponentsBuilder.fromPath("/{id}").buildAndExpand(value.wallet().getId()).toUri();
  ...
  public ResponseEntity<Response<List<WalletResponse>, Void>> all(
      @AuthenticationPrincipal User user) {
    var wallets =
        this.walletService.findAll(user).stream()
