...
@OpenAPIDefinition(
        tags = {@Tag(
                name = "Autorização e Autenticação", description = "rotas de login, cadastro e renovação de token"), @Tag(name = "Usuário", description = "Dados do usuario"), @Tag(
                        name = "Conta do Banco", description = "cadastro das contas de banco onde as transações foram feitas"), @Tag(name = "Categoria", description = "Categoria das transações financeiras"), @Tag(name = "Transações", description = "Transações financeiras"), @Tag(
...
public class OpenApiConfig {
    ...
    public OpenAPI openApi(BuildProperties properties) {
        return new OpenAPI().info(this.info(properties)).addSecurityItem(this.securityRequirement()).components(this.components());
    }

    private Info info(BuildProperties properties) {
        return new Info().title(properties.getName()).description(
                """
    ...
    private SecurityScheme securityScheme() {
        return new SecurityScheme().name(SECURITY_SCHEME_NAME).type(SecurityScheme.Type.HTTP).scheme("bearer").bearerFormat("JWT").in(SecurityScheme.In.HEADER);
    }
}
